...
check('og-desc-longer-than-meta', len(re.search(r'property="og:description"\s+content="([^"]+)"', html).group(1)) > len(re.search(r'<meta\s+name="description"\s+content="([^"]+)"', html).group(1)))

# NEW: no inline styles
check('no-inline-styles', 'style="' not in html)

# NEW: no privacy/tos dead links
check('no-privacy-link', 'href="https://neohiro.github.io/privacy/"' not in html)
check('no-tos-link', 'href="https://neohiro.github.io/tos/"' not in html)
...
check('css-glide-mask-fallback', '::before' in css and '::after' in css and 'linear-gradient' in css)
check('css-glide-hover-pause', 'animation-play-state: paused' in css)
check('css-footer-button-row', '.footer-button-row' in css)
check('css-footer-button-row-margin', 'margin-top: 0.6rem' in css)
check('css-color-mix-fallback', 'background: var(--color-surface);' in css and 'background: linear-gradient' in css)
